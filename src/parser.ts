...
export class MpParser {
  ...
  public get_piece(reverse: boolean = false): string {
    ...
    result.push('Piece UNIVERSAL PRIORITY'); counter = 1;
    this.piece_universal_priority.forEach((el)=>{
    ...
    result.push('Piece UNIVERSAL'); counter = 1;
    this.piece_universal.forEach((el)=>{
      let elem = `[${count(counter++)}] ${el.name} (${type_dict[(el.type & 30) + '']})`
      reverse ? result.unshift(elem) : result.push(elem);
    })
  ...
  protected parse(str: string) {
    ...
    let indent = -1;
    let pflag = false;
    let protect_stack: string[] = [];  // エスケープした文字列を保存する

    for(let a = 0; a < list.length; a++) {
      // インデント処理
      if (indent == -1) { indent = list[a].match(/^( )*/)[0].length; }
      for (let i = 0; i < indent; i++) { list[a] = list[a][0]==' ' ? list[a].slice(1) : list[a]; }
    }
        ...
        if(match) {
          if(el.type === MatchType.BLOCK) {
            ...
            for(let i = index + 1; i < list_length; i++) {
              if(list[i].match(el.regex[1])) { result = el; result.index = i + 1; flag = true; break;  }
            }
            if(result === null && !el.strict) { result = el; result.index = list.length; flag = true; }
            
          } else if(el.type === MatchType.BLOCK_EXCLUDE) {
            let list_length = el.limit_lines !== 0 ? Math.min(index + el.limit_lines, list.length) : list.length;
            for(let i = index + 1; i < list_length; i++) {
              if(!list[i].match(el.regex[1])) { result = el; result.index = i; flag = true; break;  }
            }
            if(result === null && !el.strict) { result = el; result.index = list.length; flag = true; }
  ...
  public standard_apply_piece = (block: string[], insert_tag: string = 'p' ) => {
    ...
    for(let a = 0; a < block.length; a++) {
      ...
      if(piece) {
        ...
        if (piece.type & (MatchType.BLOCK + MatchType.BLOCK_EXCLUDE)) {
          ...
          for(let i = a; i < index; i++) { block[i] = ""; }

          if (result) {
            if (typeof result === 'string') { block[index - 1] = result; }
            else { block[index - 1] = result.result; protect_stack.push(result.embed); }
            block_flag = true;
            line_flag = true;
          }

          if (pflag) { block[a-1] = block[a-1] + end_insert_tag; pflag = false; }

        } else if (piece.type & MatchType.LINE) {
          let result = piece.match_callback([block[a]]);
          if(typeof result === 'string') { block[a] = result; }
          else { block[index - 1] = result.result; protect_stack.push(result.embed); }
          line_flag = true;
          if(pflag) { block[a - 1] = block[a - 1] + end_insert_tag; pflag = false; }
      ...
      while(pieceu_p) {
        ...
        if(typeof result === 'string') { block[a] = block[a].replace(target, result); }
        else { block[a] = block[a].replace(target, result.result); protect_stack.splice(protect_count_before + protect_count, 0, result.embed) }
      ...
      if(!line_flag) {
        // pタグ付与
        if(a == 0) {
          if (block[a] !== "") { block[a] = start_insert_tag + block[a]; pflag = true; }

        } else if (block[a-1] === "") {
          if(block[a] === "") {
            block[a-1] = "<br>";
          } else {
            block[a] = start_insert_tag + block[a]; pflag = true;
          }
        
        } else {
          if(pflag && block[a] === "") {
            block[a-1] = block[a-1] + end_insert_tag; pflag = false;
          }
    ...
    if(pflag) { block[block.length-1] = block[block.length-1] + end_insert_tag; }
   ...
   * @returns プロテクト文字
   */
  public get_protect_sign(): string { return this.protect_sign; }
  ...
  public finally_callback() {
    ...
    piece.forEach((el) => {
      if (el.finally_callback) { el.finally_callback(); }
    })
  }
  ...
  private code_block = (match: string[]): MpString => {
    ...
    for(let i = start; i < match.length - end; i++){
      for(let j = 0; j < space; j++) { if(match[i][0] === " ") { match[i] = match[i].slice(1); } else { break; }  }
  ...
  private table = (match: string[]): MpString => {
    ...
    if(match.length > 1 && match[1].match(/ *\:*---+\:* *\|/g)) {
      ...
      for(let i = 0; i < row.length; i++) {
        if(row[i].match(/ *---+\:\: *\|/)) { align_td.push(' style="text-align:right;"'); align_th.push(' style="text-align:right;"'); }
        else if(row[i].match(/ *\:\:---+ *\|/)) { align_td.push(' style="text-align:left;"'); align_th.push(' style="text-align:left;"'); }
        else if(row[i].match(/ *\:---+\: *\|/)) { align_td.push(' style="text-align:center;"'); align_th.push(' style="text-align:center;"'); }
        else if(row[i].match(/ *---+\: *\|/)) { align_td.push(' style="text-align:right;"'); align_th.push(' style="text-align:center;"'); }
        else { align_td.push(' style="text-align:left;"'); align_th.push(' style="text-align:center;"'); }
      }
      config_exist = true;
      ...
      for(let i = 0; i < row.length; i++) { align_td.push(' style="text-align:left;"'); align_th.push(' style="text-align:center;"'); }

    }
    
    // 設定項目が存在 かつ ヘッダー行に何もない場合にはフラグを立てる
    if(config_exist && match[0].match(/^\|( *\|)+ *$/)) { header_exist = false; }
    ...
    for(let i = 0; i < match.length; i++){
      ...
      while(pieceu_p) {
        ...
        let result = pieceu_p.match_callback([target]);
        if(typeof result === 'string') { match[i] = match[i].replace(target, result); }
        else { match[i] = match[i].replace(target, result.result); protect_stack.splice(protect_count, 0, result.embed) }
      ...
      for(let j = row.length - 1; j >= 0; j--) {
        if(row[j].match(/^ *:: *$/)) { row[j] = ""; colspan++; continue; }
        if(align_td.length - 1 < j) { row[j] = ""; continue; }
        ...
        while(pieceu) {
          ...
          let result = pieceu.match_callback([target]);
          if(typeof result === 'string') { row[j] = row[j].replace(target, result); }
          else {
            row[j] = row[j].replace(target, result.result);
            let protect_count_before = 0;
            for(let k = 0; k < j; k++) { protect_count_before += protect_row_count[k]; }  // 現在の列以前に含まれる秘匿文字を計算
    ...
    if(match.length > 1) { match[1] = "<tbody>" + match[1]; match[match.length - 1] = match[match.length - 1] + "</tbody>"; }
    ...
    for(let i = 0; i < match.length; i++){
      if(match[i].match(/^(\* *\* *\* *(\*| )*|_ *_ *_ *(_| )*|- *- *- *(-| )*)$/)) {
        ...
        for(let a = i + 1; a < match.length; a++) {
          if(!match[a].match(/(^  |^ *$)/)) { block = match.slice(i, a); break; }
        }
        if(block.length === 0) { block = match.slice(i, match.length); }
        ...
        // ブロックの結合
        for(let a = i; a < i + block_length; a++) { match[a] = ""; }
