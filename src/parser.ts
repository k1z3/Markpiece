...
export class MpParser {
  ...
  public reset_piece(): void {
    ...
    this.add_piece('ul', [/^[-+*] .*/, /(^([-+*] | {2}).*|^  +$)/], this.ulist, MatchType.BLOCK_EXCLUDE) // ulタグ
    this.add_piece('ol', [/^\d+\. .*/, /(^(\d+\. | {2}).*|^  +$)/], this.olist, MatchType.BLOCK_EXCLUDE) // olタグ

    this.add_piece('blockquote', [/^>+ /, /^>+( *| +.+)$/], this.blockquote, MatchType.BLOCK_EXCLUDE) // bockquoteタグ
    this.add_piece('code_block(space)', [/^ {3} +.*/, /^ {3} +.*/], this.code_block, MatchType.BLOCK_EXCLUDE) // code-block
    ...
    this.add_piece('italic', [/\*.+?(\*\*\*|(?<!\*)\*(?!\*))|_.+?(___|(?<!_)_(?!_))/], this.italic, MatchType.UNIVERSAL) // iタグ
  ...
  public add_piece(
    ...
    finally_callback: () => any = null,
    limit_lines: number = 0,
    strict: boolean = false
  ): void {
    const piece = new Piece(name, regex, match_callback, matchtype, finally_callback, limit_lines, strict)
   ...
   * @param matchtype ルールの形式
   */
  public remove_piece(index: number, matchtype: number): void {
   ...
   * @param strict 厳密なルール適用
   */
  public rewrite_piece(
    name: string,
    regex: RegExp[],
    match_callback: (arg: string[]) => MpString | string | null,
    matchtype: number = MatchType.UNIVERSAL,
    insert_index: number,
    finally_callback: () => any = null,
    limit_lines: number = 0,
    strict: boolean = false
   ...
   */
  public get_piece(reverse: boolean = false): string {
    const result: string[] = []
    const type_dict: { [key: string]: string } = {
      '2': 'UNIVERSAL', // 単語
      '4': 'LINE', // 単一行
      '8': 'BLOCK', // 複数行
    ...
    this.piece.forEach((el) => {
      const elem = `[${count(counter++)}] ${el.name} (${type_dict[(el.type & 30) + '']})`
    ...
    counter = 1
    this.piece_universal_priority.forEach((el) => {
      const elem = `[${count(counter++)}] ${el.name} (${type_dict[(el.type & 30) + ' PRIORITY']})`
      reverse ? result.unshift(elem) : result.push(elem)
    })
    result.push('\n')
   ...
   */
  public add_emoji(name: string, content: string, type: string = 'str'): void {
   ...
   */
  public get_emoji_table(): { [index: string]: { content: string; type: string } } {
  ...
  public piece_match = (
    ...
    index: number,
    piece: Piece[] = null,
    matchtype: number = MatchType.BLOCK + MatchType.BLOCK_EXCLUDE + MatchType.LINE + MatchType.UNIVERSAL
  ): Piece | null => {
    let flag = false
    ...
    piece.forEach((el) => {
      if (!flag && matchtype & el.type) {
        /* matchを探索する */
        const match = list[index].match(el.regex[0])

        if (match) {
          if (el.type === MatchType.BLOCK) {
            el.index = list.length
            const list_length = el.limit_lines !== 0 ? Math.min(index + el.limit_lines, list.length) : list.length
    ...
    const match_univ_priority = list[index].match(this.universal_priority_regex)

    if (!flag && match_univ_priority !== null) {
      this.piece_universal_priority.forEach((el) => {
        if (!flag && matchtype & el.type) {
          /* matchを探索する */
          const match = match_univ_priority[0].match(el.regex[0])
          if (match) {
            result = el
            flag = true
          }
    ...
    const match_univ = list[index].match(this.universal_regex)

    if (!flag && match_univ !== null) {
      this.piece_universal.forEach((el) => {
        if (!flag && matchtype & el.type) {
          /* matchを探索する */
          const match = match_univ[0].match(el.regex[0])
          if (match) {
            result = el
            flag = true
          }
   ...
   * @returns Piece適用後の結果
   */
  public standard_apply_piece = (block: string[], insert_tag: string = 'p') => {
    const start_insert_tag = insert_tag ? `<${insert_tag}>` : ''
    const end_insert_tag = insert_tag ? `</${insert_tag}>` : ''

    const protect_stack: string[] = []
    let pflag = false

    // markdown 解析
    for (let a = 0; a < block.length; a++) {
      const piece = this.piece_match(block, a, null, MatchType.BLOCK + MatchType.BLOCK_EXCLUDE + MatchType.LINE) // match する Piece があるかを検索する
      const protect_count_before = protect_stack.length
      ...
      if (piece) {
        ...
        if (piece.type & (MatchType.BLOCK + MatchType.BLOCK_EXCLUDE)) {
          const result = piece.match_callback(block.slice(a, index))
          ...
          const result = piece.match_callback([block[a]])
          if (typeof result === 'string') {
            block[a] = result
          } else {
            block[index - 1] = result.result
      ...
      let count = 0
      while (pieceu_p) {
        const match = block[a].match(pieceu_p.regex[0])
        const target = match[0]
        const protect_count = (block[a].slice(0, match.index).match(RegExp(this.get_protect_sign(), 'g')) || []).length
        const result = pieceu_p.match_callback([target])
        if (typeof result === 'string') {
      ...
      count = 0
      while (pieceu) {
        const match = block[a].match(pieceu.regex[0])
        const target = match[0]
        const protect_count = (block[a].slice(0, match.index).match(RegExp(this.get_protect_sign(), 'g')) || []).length
        const result = pieceu.match_callback([target])
        if (typeof result === 'string') {
          block[a] = block[a].replace(target, result)
        } else {
          block[a] = block[a].replace(target, result.result)
    ...
    // protectした文字を元に戻す
    protect_stack.forEach((el: string) => {
  ...
  protected concat_regexp_or(reg_exps: RegExp[], flags?: string): RegExp {
    ...
    return RegExp(
      reg_exps.reduce((acc, cur) => acc + cur.source, ''),
      flags
    )
  }
   ...
   */
  public escape_html(str: string): string {
   ...
   * @returns プロテクト文字
   */
  public get_protect_sign(): string {
    return this.protect_sign
  }

  /**
   * Piece に紐づけられた finally_callback を呼び出す
   */
  public finally_callback() {
    const piece = this.piece.concat(this.piece_universal, this.piece_universal_priority)
  ...
  /* =========== */
  private code_block = (match: string[]): MpString => {
    const space = match[0].match(/^\s*/).toString().length
    const start = match[0].match(/^ {3} +.*/) ? 0 : 1
    const end = match[match.length - 1].match('```') ? 1 : 0
    ...
    const result = new MpString(
      this.get_protect_sign(),
      '<pre><code>' + match.slice(start, match.length - end).join('\n') + '</code></pre>'
    )

    return result
  }

  private table = (match: string[]): MpString => {
    const align_th: string[] = []
    const align_td: string[] = []
    let config_exist: boolean = false
    let header_exist: boolean = true

    // 2番目の設定の存在確認と取得
    if (match.length > 1 && match[1].match(/ *:*---+:* *\|/g)) {
      const row = match[1].match(/ *:*---+:* *\|/g)
      ...
      const row = match[0].match(/.*?\|/g).slice(1)
      for (let i = 0; i < row.length; i++) {
        align_td.push(' style="text-align:left;"')
        align_th.push(' style="text-align:center;"')
      }
    ...
    for (let i = 0; i < match.length; i++) {
      ...
      const protect_stack: string[] = []
      const protect_row_count: number[] = []
      let pieceu_p = this.piece_match([match[i]], 0, null, MatchType.UNIVERSAL + MatchType.PRIORITY) // match する pieceu があるかを検索する
      while (pieceu_p) {
        const select = match[i].match(pieceu_p.regex[0])
        const target = select[0]
        const protect_count = (match[i].slice(0, select.index).match(RegExp(this.get_protect_sign(), 'g')) || []).length
        const result = pieceu_p.match_callback([target])
        if (typeof result === 'string') {
      ...
      // 行の取得と前処理
      const row = match[i].match(/.*?\|/g).slice(1)
      let colspan = 1
      ...
      for (let j = row.length - 1; j >= 0; j--) {
        ...
        while (pieceu) {
          const match = row[j].match(pieceu.regex[0])
          const target = match[0]
          const protect_count = (row[j].slice(0, match.index).match(RegExp(this.get_protect_sign(), 'g')) || []).length
          const result = pieceu.match_callback([target])
          if (typeof result === 'string') {
        ...
        row[j] =
          i === 0 && config_exist
            ? `<th${align_th[j]} colspan="${colspan}">${row[j]}</th>`
            : `<td${align_td[j]} colspan="${colspan}">${row[j]}</td>`
        colspan = 1
      }
    ...
    const result = new MpString(this.get_protect_sign(), '<table>' + match.join('') + '</table>')

    return result
  }

  private ulist = (match: string[]): MpString => {
    // 再帰的にリスト内要素を探索する
    for (let i = 0; i < match.length; i++) {
      if (match[i].match(/^(\* *\* *\* *(\*| )*|_ *_ *_ *(_| )*|- *- *- *(-| )*)$/)) {
        match[i] =
          '</ul>' + match[i].replace(/^(\* *\* *\* *(\*| )*|_ *_ *_ *(_| )*|- *- *- *(-| )*)$/, '<hr>') + '<ul>'
        ...
        const block_length = block.length
        const tmp = this.standard_apply_piece(block)
    ...
    const result = new MpString(this.get_protect_sign(), '<ul>' + match.join('') + '</ul>')

    return result
  }

  private olist = (match: string[]): MpString => {
    const start_counter = match[0].match(/^\d+/)[0]
    ...
    const result = new MpString(this.get_protect_sign(), `<ol start='${start_counter}'>` + match.join('') + '</ol>')

    return result
  }

  private blockquote = (match: string[]): MpString => {
    // const br_flag = false
    // const protect_stack: string[] = []
    ...
    const tmp = this.standard_apply_piece(match)

    const result = new MpString(this.get_protect_sign(), '<blockquote>' + tmp + '</blockquote>')

    return result
  }

  private h1 = (match: string[]): string => {
    let target: string
  ...
  private h2 = (match: string[]): string => {
    let target: string
  ...
  private h3 = (match: string[]): string => {
    let target = match[0]
  ...
  private h4 = (match: string[]): string => {
    let target = match[0]
  ...
  private h5 = (match: string[]): string => {
    let target = match[0]
  ...
  private h6 = (match: string[]): string => {
    let target = match[0]
  ...
  private hr = (match: string[]): string => {
    let target = match[0]
  ...
  private italic = (match: string[]): string => {
    let result = ''
    ...
    if (match[0].match(/\*\*\*$|___$/)) {
      if (match[0].slice(1, match[0].length - 3).match(new RegExp(`\\${char}`))) {
        ...
        result = `<i>${match[0].slice(1, match[0].length - 3)}</i>${char}${char}`
      }
    } else {
      result = `<i>${match[0].slice(1, match[0].length - 1)}</i>`
    }

    return result
  }

  private bold = (match: string[]): string => {
    let result = match[0]
    ...
    if (match[0].match(/.+\*\*\*$|.+___$/)) {
      if (result.slice(0, result.length - 3).match(new RegExp(`\\${char}`))) {
        ...
        result = `${result.slice(0, result.length - 3)}</b>${char}`
      }
    } else {
      result = `${result.slice(0, result.length - 2)}</b>`
    }

    return result
  }

  private strikethrough = (match: string[]): string => {
    const result = `<s>${match[0].slice(2, match[0].length - 2)}</s>`

    return result
  }

  private insert = (match: string[]): string => {
    const result = `<ins>${match[0].slice(2, match[0].length - 2)}</ins>`

    return result
  }

  private mark = (match: string[]): string => {
    const result = `<mark>${match[0].slice(2, match[0].length - 2)}</mark>`

    return result
  }

  private code = (match: string[]): MpString => {
    const result = new MpString(
      this.get_protect_sign(),
      `<code>${this.escape_html(match[0].slice(1, match[0].length - 1))}</code>`
    )

    return result
  }

  private escape = (match: string[]): MpString => {
    const result = new MpString(this.get_protect_sign(), this.escape_html(match[0][1]))

    return result
  }

  private image = (match: string[]): string => {
    // const search = match[0].match(/!\[.+?\]\(.+?\)/)[0]
    const alt = match[0].match(/!\[.+?\]/)[0]
    const src = match[0].match(/\(.+?\)/)[0]

    const result = `<img src='${src.slice(1, src.length - 1)}' alt='${alt.slice(2, alt.length - 1)}'>`

    return result
  }

  private anker = (match: string[]): MpString => {
    const search = match[0].match(/\[.+?\]\(.+?\)/)[0]
    const tag = search.match(/\[.+?\]/)[0]
    const href = search.replace(/\[.+?\]/, '').match(/\(.+?\)/)[0]

    const result = `<a href='${href.slice(1, href.length - 1)}'>${tag.slice(1, tag.length - 1)}</a>`

    return new MpString(this.get_protect_sign(), result)
  }

  private sup = (match: string[]): string => {
    const result = `<sup>${match[0].slice(1, match[0].length - 1)}</sup>`

    return result
  }

  private sub = (match: string[]): string => {
    const result = `<sub>${match[0].slice(1, match[0].length - 1)}</sub>`

    return result
  }

  private emoji = (match: string[]): string => {
    let target = match[0].match(/::?[0-9a-zA-Z_\-+]+?::?/)[0]

    if (target.match(/::[0-9a-zA-Z_\-+]+?::/)) {
      const search = target.match(/::[0-9a-zA-Z_\-+]+?::/)
      ...
      if (target in emoji) {
        if (emoji[target].type === 'str') {
          ...
          target = match[0].replace(
            search[0],
            `<img src='${emoji[target].content}' style='width:auto; height:2em; border-radius:0; vertical-align:sub;'>`
          )
        }
      } else {
      ...
      if (target in emoji) {
        if (emoji[target].type === 'str') {
          ...
          target = match[0].replace(
            search[0],
            `<img src='${emoji[target].content}' style='width:initial; height:1.3em; border-radius:0; vertical-align:sub;'>`
          )
        }
      } else {
        target = match[0].replace(search[0], target)
